...
function checkArrayAscending(arr) {
  let comparisonValue = arr[0] // set comparison value to the first index to see if the next number is bigger or smaller
  for (let i = 1; i < arr.length - 1; i++) {
    if (arr[i] <= comparisonValue) { // if the value at the particular index is smaller than the comparison value, then it's descending.
      return false
    } else {
